...
import { addUpdateUser } from '@/lib/db-actions'
...
<Form {...form}  >
    ...
    render={({ field }) => (
        ...
        <FormItem className=" flex justify-center  gap-20 flex-wrap max-md:gap-10 ">
            ...
            <FormLabel className=" mr-8 relative overflow-hidden
             w-[250px] flex justify-center items-center m-0 !h-[250px] 
            bg-gray-900 rounded-full flexcenter ">

                {field?.value ? ( 

                <Image src={field.value}  className='object-cover'
                alt="image of you" fill />)

                :(<Image src="/assets/profile.svg"
                    className=" object-contain" alt="image" height={70} width={70}/>)
...
<FormField
    ...
    render={({ field }) => (
       ...
       Bio
        ...
        <FormControl className="">
            <Input className="account-form_input " type='text'  {...field as InputProps} />
        </FormControl>
        <FormMessage />
    </FormItem>
